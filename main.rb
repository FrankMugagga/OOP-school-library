...
def handle_option(option, app)
    ...
    when :list_all_books, :list_all_people, :create_book, :create_rental
        app.send(handler)
    when :list_person_rentalsperson_id = prompt_input('Enter the person ID:').to_i
        app.send(handler, person_id)
    when :create_person
        create_person_prompt(app)
    when :Exitexit_program
        return false
    else
        invlid_option
    end
...
def main
    ...
    loop do
        list_all_options
        option = gets.chomp.to_ibreak unless handle_option(option, app)
    end
end
